    ...
    for key_name, key_attributes in json_model['properties'].items():
        ...
        if key_attributes['type'] == 'string' and 'enum' in key_attributes:
            ...
            if len(key_attributes['enum']) == 1:
                print("WARNING: enum value with single choice for field {}, choice {}.".format(key_name, key_attributes['enum']))
                continue
        ...
        # Date time field
            ...
            field_str = "    {} = models.DateTimeField({})\n" \
                        "".format(key_name, required_str)

        elif key_attributes['type'] == 'string':
            field_str = "    {} = models.CharField({})\n".format(key_name, required_str)

        elif key_attributes['type'] == 'integer':
            field_str = "    {} = models.IntegerField({})\n".format(key_name, required_str)

        elif key_attributes['type'] == 'base_decimal':
            field_str = f'    {key_name} = models.DecimalField({required_str}, max_digits=4, decimal_places=2)\n'

        elif key_attributes['type'] == 'micro_decimal':
            field_str = f'    {key_name} = models.DecimalField({required_str}, max_digits=6, decimal_places=5)\n'
