...
def test_drf_pagination():
    ...
    client = Client()
    response = client.get('/api/foodproducts/search/')
    data = response.json()
...
def test_drf_search_food_products_q_param():
    ...
    client = Client()
    response = client.get('/api/foodproducts/search/', data={'q': 'eggs'})
    data = response.json()
...
def test_drf_search_detailed_food_products_auth():
    ...
    response = client.get('/api/foodproducts/search_detailed/')
